"""Batch-driver contract for the EVENT_DAY stage and for evidence
persistence coverage.

Two separate concerns live here:

1. **The stage itself** — capture today's board, evaluate it under the
   EVENT_DAY declaration, and settle it in its own tree. Every failure
   mode must stay inside the stage (the driver's isolation contract) so a
   event-day problem can never cost the frozen 24h pipeline a dispatch.
...
class TestShortNoticeStage:
    def test_constants_point_at_the_separate_tree_and_declaration(self):
        assert fsb.EVENT_DAY_SHADOW_SUBDIR == "shadow_event_day"
        assert fsb.STANDARD_SHADOW_SUBDIR == "shadow"
        assert fsb.EVENT_DAY_CONFIG == (
            "config/shadow_evaluator_event_day.json")
        assert Path(fsb.EVENT_DAY_CONFIG).is_file()

    def test_find_event_day_run_ignores_the_frozen_tree(self, tmp_path):
        _write_run(tmp_path, "shadow", TARGET_DATE, RUN_ID, [])
        assert fsb.find_event_day_run(TARGET_DATE, tmp_path) is None
        _write_run(tmp_path, "shadow_event_day", TARGET_DATE, "abc123", [])
        assert fsb.find_event_day_run(TARGET_DATE, tmp_path) == "abc123"

    def test_find_event_day_run_skips_blocked_and_incomplete(self, tmp_path):
        day = tmp_path / "data" / "reports" / "shadow_event_day" / TARGET_DATE
    ...
    def test_summary_counts_one_r1_per_sport(self, tmp_path):
        run_dir = _write_run(
            tmp_path, "shadow_event_day", TARGET_DATE, RUN_ID,
            [_selection("football", 1), _selection("football", 2),
             _selection("basketball", 1), _selection("hockey", 1),
             _selection("hockey", 3)])
        summary = fsb.summarise_event_day_run(run_dir)
    ...
    def test_dry_run_touches_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("slumdog.forebet.ForebetCollector", _boom)
        entry = fsb.run_event_day_for_date(
    ...
    def test_capture_failure_is_isolated_never_raised(self, tmp_path, monkeypatch):
        ...
        entry = fsb.run_event_day_for_date(TARGET_DATE, tmp_path)
        assert entry["status"] == "EVENT_DAY_FAILED"
    ...
    def _run_stage_with_fake_evaluator(self, tmp_path, monkeypatch, *,
        ...
        The evaluator's artifact dir is written OUTSIDE the event-day tree
        ...
        (run_dir / "manifest.json").write_text(json.dumps({
            ...
            "event_day_timing_rejections": rejections or {},
        }))
        seen: dict = {}
        ...
        return fsb.run_event_day_for_date(TARGET_DATE, tmp_path), seen
...
class TestSettlementTreeScoping:
    ...
    def test_d_plus_1_rule_holds_on_the_event_day_tree(self, tmp_path):
        _write_run(tmp_path, "shadow_event_day", TARGET_DATE, "bbbb", [])
        assert fsb.find_settleable_dates(
    ...
    def test_settle_run_receives_the_subdir(self, tmp_path, monkeypatch):
        ...
        assert seen["shadow_subdir"] == fsb.EVENT_DAY_SHADOW_SUBDIR
...
class TestPersistedEvidenceDeclaration:
    ...
    def test_every_artifact_the_event_day_stage_writes_is_declared(self):
        ...
        for filename in ("shadow_selections.json", "manifest.json",
            ...
            assert ("data/reports/shadow_event_day", filename) in declared
...
class TestWorkflowGlobCoverage:
    ...
    def test_coverage_requires_both_root_and_name_to_match(self):
        ...
        assert not is_covered(
            "data/reports/shadow_event_day", "settlement_x.json", rules)
    ...
    def test_sibling_tree_is_not_covered_by_a_prefix_name_collision(self):
        ...
        ``data/reports/shadow_event_day`` — string-prefix matching without
        ...
        rules = parse_persist_rules(
            ...
            "| xargs -r git add -f")
        assert not is_covered(
            "data/reports/shadow_event_day", "manifest.json", rules)
...
class TestSettlementEvidenceSeparation:
    def test_receipt_name_is_per_track(self):
        ...
        event_day = settlement_receipt_name(EVENT_DAY_SHADOW_SUBDIR)
        ...
        assert event_day != standard
        assert EVENT_DAY_SHADOW_SUBDIR in event_day
    ...
    def test_event_day_settlement_writes_its_own_receipt_file(
            self, tmp_path, monkeypatch):
        """The event-day settlement must not reuse — and therefore must
        ...
        result = ss.settle_run(
            ...
            shadow_subdir="shadow_event_day",
        ...
        assert (evidence
                / "settlement_capture_receipt_shadow_event_day.json").is_file()
