...
**Milestone 4 architecture COMPLETE and verified (262 tests), now 4E hardening COMPLETE and verified (305 tests), now final integrity COMPLETE and verified (323 tests) — dataset.py hardened with disposition vocabulary SETTLED/SETTLED_CUP/SETTLED_DRAW/VOID/NO_CONTEST, winner_index strict int (bool/float/string rejected), deterministic provenance merge (identical collapses, missing vs present preserves present, different non-empty hashes/URLs fail loudly, stable under reversed order), source-conflict limitation documented, raw vs canonical accounting, strengthened digest, duplicate identity validated, provenance validated, no unsafe defaults, no silent swallowing**
...
- Milestone 4 (branch, current): `src/slumdog/dataset.py` hardened — `PriceFreeUnderdogExample` contract, `PriceFreeDatasetReceipt` with raw/canonical accounting (raw_input_rows, schema_excluded_rows, valid_loaded_rows, exact_duplicates_collapsed, canonical_input_rows, eligible_examples, builder_excluded_rows, provenance_invalid, canonical_date_min/max, eligible_date_min/max), builder `build_price_free_examples` + `build_dataset_with_raw_accounting` + `load_settled_events_from_dicts` + `_validate_settled_dict` no unsafe defaults, disposition vocabulary SETTLED/SETTLED_CUP/SETTLED_DRAW/VOID/NO_CONTEST from settlement.py unknown dispositions schema-excluded, winner_index strict int bool/float/string rejected, `_canonical_event_repr` versioned including all affecting fields excluding odds deliberately, `_compute_input_digest` stable under reordering, `_is_valid_sha256` 64-hex, composite key (sport,event_id,event_date) exact collapse vs conflict fail loudly, deterministic provenance merge identical collapses missing vs present preserves present different non-empty hashes/URLs fail loudly stable under reversed order, source-conflict limitation documented, provenance validation, date semantics explicit, minimal safe feature set ALLOWED only, missingness policy, timing guarantees, eligibility rules, receipt invariants raw=schema+valid valid=exact+canonical canonical=eligible+builder, price-independence tests, 30 tests in `tests/test_price_free_dataset.py` + 34 hardening in `tests/test_dataset_hardening.py` + 9 audit in `tests/test_dataset_audit.py` + 18 final integrity in `tests/test_dataset_final_integrity.py`, full suite 323 passed, training frozen, no integration into legacy training.py yet
- Milestone 4E + final integrity (branch, current): `src/slumdog/dataset_audit.py` entry point `python -m slumdog.dataset_audit --root data --receipt /tmp/slumdog_price_free/receipt.json --sample /tmp/slumdog_price_free/examples_sample.json --sample-size 5` — no network, writes only under /tmp, tested adapters for settled_history.json (list) and history_*.jsonl.gz (JSONL gz with facets raw_sha256), explicit NO_SUPPORTED_INPUT_FILES status vs fail on unreadable/corrupt/unknown schema/conflicting duplicates including provenance conflicts, malformed rows visible by reason, never defaults missing winner, never silently skips, prints summary counts, full receipt with zeros when no files
...
- `src/slumdog/dataset.py` hardened final — PriceFreeUnderdogExample, PriceFreeDatasetReceipt with raw/canonical accounting, builder with raw accounting, schema adapter _validate_settled_dict no defaults, disposition vocabulary SETTLED/SETTLED_CUP/SETTLED_DRAW/VOID/NO_CONTEST from settlement.py unknown dispositions schema-excluded, winner_index strict int bool/float/string rejected, canonical repr versioned, digest stable under reordering odds excluded deliberately source-conflict limitation documented, duplicate identity composite key (sport,event_id,event_date), deterministic provenance merge identical collapses missing vs present preserves present different non-empty hashes/URLs fail loudly stable under reversed order, provenance 64-hex validation, date semantics explicit, minimal safe feature set ALLOWED only, missingness policy, timing guarantees, eligibility rules, receipt invariants, price-independence, 30 tests + 34 hardening + 9 audit + 18 final integrity = 323 total
- `src/slumdog/dataset_audit.py` entry point python -m slumdog.dataset_audit --root data --receipt /tmp/slumdog_price_free/receipt.json --sample /tmp/slumdog_price_free/examples_sample.json --sample-size 5 — no network, writes only under /tmp, tested adapters for settled_history.json and history_*.jsonl.gz, explicit NO_SUPPORTED_INPUT_FILES vs fail on unreadable/corrupt/unknown schema/conflicting duplicates including provenance conflicts, malformed rows counted by reason, never defaults missing winner, never silently skips, full receipt zeros when no files
- `tests/test_price_free_dataset.py` — 30 tests price independence, draw semantics, timing, integrity, serialization
- `tests/test_dataset_hardening.py` — 34 tests missing winner never participant 1, missing disposition excluded, malformed row counted, corrupt JSON/gzip fails, unknown schema version fails, raw/canonical invariants, exact duplicate accounting, conflicting duplicate failure, digest changes on prob/winner/score, stable on reordering, odds not changing digest, malformed SHA counted, all-excluded date semantics, duplicate identity same event_id different sports, same participants/date different leagues, provenance change handling deterministic merge
...
- `tests/test_dataset_final_integrity.py` — 18 tests unknown disposition schema-excluded, missing/empty disposition schema-excluded, winner_index True/False/float/string rejected, supported void/no-contest explicitly accounted, canonical disposition vocabulary, provenance merge stable under reversed order, missing plus valid preserves valid, different valid hashes conflict, different source URLs conflict, digest stable after duplicate normalization, source-conflict limitation documented and eligibility changes where supported
- `docs/PRICE_FREE_DATASET_CONTRACT.md` — concise hardened final contract with exact versions, allowed/prohibited, missingness, timing, receipt accounting with invariants, disposition vocabulary, deterministic provenance merge, source-conflict limitation, adapter schemas, Codespace command hardened final

Next after Milestone 4 approval: Milestone 5 readiness review — transparent baselines planning (Forebet underdog prob, gap, recent-form differential, Ma Golide heuristic, simple interpretable model) with walk-forward validation, never random splits, after user approves dataset contract and real-data receipt execution. Training remains frozen. PR #6 remains open and unmerged.
...
## Verification Receipt (Milestone 0-4E + Final Integrity)
...
- Final integrity hardening: `src/slumdog/dataset.py` updated — disposition vocabulary SUPPORTED_DISPOSITIONS = {SETTLED, SETTLED_CUP, SETTLED_DRAW, VOID, NO_CONTEST} derived from settlement.py (SETTLED, SETTLED_CUP, SETTLED_DRAW, VOID) plus NO_CONTEST alias, unknown dispositions PENDING/LIVE/ABANDONED/CANCELLED/POSTPONED/FT/arbitrary → SCHEMA_UNKNOWN_DISPOSITION schema-excluded, missing/empty disposition → SCHEMA_MISSING_DISPOSITION, winner_index strict int 0/1/2 bool rejected (type is bool check) float/string rejected (type is int required), deterministic provenance merge identical collapses missing vs present preserves present deterministically different non-empty hashes fail loudly different non-empty source URLs fail loudly stable under reversed input order, digest stable after duplicate normalization, source-conflict limitation documented (SettledEvent does not represent source conflict, not in digest, builder assumes no conflict, receipt excluded_source_conflict 0 for current schemas), 18 final integrity tests, full suite 323 passed
- Tests: `python3 -m pytest -q` 323 passed, `python3 -m pytest -q tests/test_price_free.py tests/test_price_free_dataset.py tests/test_dataset_hardening.py tests/test_dataset_audit.py tests/test_dataset_final_integrity.py` 131 passed, `python3 -m py_compile src/slumdog/*.py tests/*.py` ok, `python3 -m pyflakes src/slumdog/dataset.py src/slumdog/dataset_audit.py src/slumdog/underdog.py` ok, `git diff --check` ok
- Training remains frozen: `MODEL_TRAINING_ALLOWED=False`
- Codespace command hardened final: `python -m slumdog.dataset_audit --root data --receipt /tmp/slumdog_price_free/receipt.json --sample /tmp/slumdog_price_free/examples_sample.json --sample-size 5` — no network, writes only under /tmp, prints summary, exits nonzero on unreadable/unknown schema/conflicting duplicates including provenance conflicts, never defaults missing winner, never silently skips, NO_SUPPORTED_INPUT_FILES explicit when no ledgers with full receipt zeros
- Real-data audit in this sandbox: NO_SUPPORTED_INPUT_FILES (data/interim/settled_history.json and data/reports/history_*.jsonl.gz absent), receipt written to /tmp/slumdog_price_free/receipt.json with status, files_found 0, raw 0, schema_excluded 0, valid 0, exact_duplicates 0, canonical 0, eligible 0, builder_excluded 0, per_sport {}, exclusion reasons {}, provenance 0/0/0, canonical and eligible date ranges null, positive rate null, receipt path /tmp/slumdog_price_free/receipt.json, sample path /tmp/slumdog_price_free/examples_sample.json
