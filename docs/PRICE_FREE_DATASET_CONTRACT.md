...
**Feature contract version:** `price-free-minimal-2026-08-24`
**Label contract version:** `price-free`
...
- **Research feature contract:** `price-free-incremental-valid-history` on every emitted example, sample, and receipt. The research-mode change is history membership: only rows passing `research_history_eligible` feed the incremental history state. The label contract is unchanged (`price-free`). Research semantics are never emitted under the strict-mode version string.
- **Intentional history-membership differences (research mode vs strict mode)** (vs the legacy `HistoryIndex` filter; tested as divergences, not equivalence): unknown-sport rows excluded (legacy included SETTLED rows); void compatibility aliases (`NO_CONTEST`) excluded (legacy included them); incoherent disposition/winner combinations excluded (e.g. `SETTLED_CUP` with `winner_index 0`, two-way draw under `SETTLED`); duplicate normalization happens before building, so self-pair double counting is structurally impossible.
...
- Digests (full 64-hex SHA-256, never truncated): per-sport input digest over LF-terminated `_canonical_event_repr` JSONL sorted by (event_date, event_id); combined `input_digest` over the exact bytes `slumdog-research-input\n` + per-sport `sport\nrow_count\ndigest\n` blocks sorted by sport; `examples_digest` over the exact emitted gzip payload bytes (incremental, no reread).
