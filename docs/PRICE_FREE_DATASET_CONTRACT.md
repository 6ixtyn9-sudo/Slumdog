...
- **Disposition vocabulary explicit (canonical from settlement.py):** `SUPPORTED_DISPOSITIONS = SETTLED_DISPOSITIONS ∪ VOID_DISPOSITIONS` where `SETTLED_DISPOSITIONS = {SETTLED, SETTLED_CUP, SETTLED_DRAW}` from `settlement.py` (FT, AOT, AP, FINAL → SETTLED; cup extra_time/penalty → SETTLED_CUP; cricket draw comment → SETTLED_DRAW) and `VOID_DISPOSITIONS = {VOID, NO_CONTEST}` (void includes no-contest, abandoned, cancelled mapped to VOID in settlement). Unknown dispositions such as `PENDING, LIVE, ABANDONED, CANCELLED, POSTPONED, FT, arbitrary` → `SCHEMA_UNKNOWN_DISPOSITION` schema-excluded unless canonical contract explicitly supports it. Supported settled → eligible for labeling; supported void/no-contest → loaded then explicitly excluded by label contract (`excluded_void`).
