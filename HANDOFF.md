...
- **`scripts/probe_kickoff_timezone.py`** (+17 offline tests): read-only evidence gathering for the timezone hold. Question 1 — is there a machine-readable start instant in the raw HTML? Could NOT be answered from the agent sandbox (Markdown-only fetch; direct HTTPS blocked; three public CORS proxies were down or rate-limited), so it needs one run from a machine with raw-HTML access. Question 2 — what offset does our relay render at? Measured by joining the tz=0 football JSON to the football HTML board on match id. Exits 0 only on a definite answer; a zero offset is reported as one observation, not a guarantee.
- **Persist coverage** now also declares `settlement_capture_receipt_shadow_event_day.json`; the §5 owner paste widens that glob to `settlement_capture_receipt*.json` and was re-verified to bring the checker to exit 0.
- **Gates:** 1203 passed, pyflakes clean, `py_compile` ok, `git diff --check` ok.

**Last updated:** 2026-09-26 (UTC, later session) — **RED-TEAM PASS ON THE EVENT_DAY TRACK BEFORE ITS FIRST DISPATCH.**

- **Blocker found and fixed: the kickoff timezone was an assumption, not a fact.** The gate's whole claim is "frozen ≥120 minutes before kickoff", which is only as good as the timezone of the kickoff we read. Only football's capture URL pins `tz=0`. A live measurement on 2026-09-26 showed HTML boards render kickoff in the requesting client's timezone (5h off UTC in the sample) and ignore `?tz=0`. Sports other than football are now refused with `KICKOFF_TIMEZONE_NOT_PROVEN_UTC` (`UTC_KICKOFF_PROVEN_SPORTS`), and the stage does not fetch those boards; they are listed as `timezone_hold_sports` in the stage receipt. The track is therefore football-only until an evidence-based calibration of the relay's rendering offset is built (design recorded in `docs/EVENT_DAY_TRACK.md` §6.1).
...
- **Pacing fixed.** `ForebetCollector.capture_selected(..., pause_seconds=)` fetches serially with the settlement capture's 62s spacing; the event-day stage passes the driver's value through, so the new daily stage does not raise the request rate.
...
- **Gates:** 1186 passed, pyflakes clean, `py_compile` ok, `git diff --check` ok. New tests: `tests/test_event_day_track.py` 74, `tests/test_event_day_batch.py` 36.
- **Unchanged:** no workflow file touched; the delta-evidence persist gap still needs the owner paste (`docs/EVENT_DAY_TRACK.md` §5); no real event-day run exists.

**Last updated:** 2026-09-26 (UTC) — **EVENT_DAY TRACK LANDED LOCALLY (owner decision 2026-09-26) + WORKFLOW EVIDENCE-GLOB GAP FOUND AND DOCUMENTED FOR OWNER PASTE.**

1. **Why.** The product promises a shortlist per sport; the frozen track has been delivering football only since 2026-09-21 (09-23 and 09-25 also had handball). Root cause is capture timing, not the model: Forebet publishes basketball / hockey / baseball / tennis / rugby / american-football boards only about a day out, so the date-anchored 24h gate (`captured_at` and `decision_committed_at` <= `target_date 00:00 UTC - 24h`) can never admit them — the T+1/T+2 refresh added on 2026-09-22 still gets `target date missing from HTML` for all of them. The owner chose (2026-09-26) a **separate, clearly-labelled event-day track** over relaxing the frozen gate.
2. **What changed (code + config + tests + docs; NO workflow file, NO frozen config, NO committed evidence).**
   - `src/slumdog/shadow_evaluator.py`: `TrackPolicy` / `track_policy()` (declaration-driven), `parse_kickoff_utc()` (both published shapes, UTC because captures pin `tz=0`; returns `None` rather than guessing), `_timing_classify_event_day()` (per-event gate with five explicit rejection reasons), event-day branch in `_emit_run()` / `evaluate_from_disk()` / `_blocked_run()`, `_validate_event_day_timing()` in `load_shadow_declaration()`, and a symmetric guard so a standard declaration cannot write into the event-day tree. Event-day-only keys (`track`, `timing_contract`, `kickoff_utc`, `lead_minutes_at_decision`, `event_day_timing_rejections`, `timing_track` in the input digest) follow the existing refresh-mode precedent: absent on standard runs, so those payloads/manifests/digests are byte-stable.
   - `src/slumdog/shadow_contracts.py`: `PreEventRecord.kickoff` (optional, defaulted, PRE_EVENT schedule field — the forbidden-field boundary is unchanged).
   - `config/shadow_evaluator_event_day.json`: new declaration, `min_lead_minutes_before_kickoff` 120, all authorization gates still false, same `rule` / `anti_tuning` / `cohort_policy` blocks as the frozen declaration (asserted by test).
   - `src/slumdog/shadow_settle.py`: `shadow_run_dir()` + `shadow_subdir=` on `load_prediction_run`, `write_settlement_artifact`, `settle_run`, `settle_selection_deltas`, `complete_settlement`, and CLI `--shadow-subdir`. Defaults keep every existing caller and artifact untouched; an unknown tree raises.
   - `scripts/forward_shadow_batch.py`: `run_event_day_for_date` / `find_event_day_run` / `summarise_event_day_run`, `--skip-event-day`, `config_rel` on `run_evaluator`, `shadow_subdir` threaded through the settlement finders/backlog, namespaced receipt keys (`event_day`, `event_day_settlement`, `event_day_*` summary counters). Stage order: D+1 settlement → completion → delta settlement → refresh → **event-day settlement + event-day capture/evaluate** → forward pass. Fully isolated (never raises to the driver).
   - `scripts/check_workflow_evidence_globs.py` + `PERSISTED_EVIDENCE` / `KNOWN_UNCOVERED_PENDING_OWNER_PASTE` in the driver: the driver now declares every small artifact it can write, and the tool reports which of them the workflow persist step would actually commit.
   - Tests: +92 (`tests/test_event_day_track.py` 61, `tests/test_event_day_batch.py` 31). `.gitignore` gained the event-day tree with the same raw-body/archive exclusions. Docs: `docs/EVENT_DAY_TRACK.md` (new), `docs/STATE.md`, `docs/README.md`, `README.md`, this file.
3. **OWNER ACTION — workflow persist step (exact paste in `docs/EVENT_DAY_TRACK.md` §5).** Two gaps: (a) the pre-existing refresh-delta loss (`selections_delta_*`, `settlement_delta_*` never committed since 2026-09-22 — verified: zero delta files exist anywhere in the repo despite four daily refresh receipts), and (b) the new `data/reports/shadow_event_day/` tree. `capture_event_day_*.json` already matches the existing `capture_*.json` rule. Verify with `python scripts/check_workflow_evidence_globs.py` — exit 1 with 10 uncovered types today, exit 0 after the paste. The suite asserts the gap is a SUBSET of the documented list, so it is green before AND after; it fails only if a new uncovered artifact type appears.
4. **Honest status.** IMPLEMENTED AND TESTED LOCALLY. **No real event-day run has been dispatched, and none has been settled** — no event-day hit rate exists and none may be quoted. Bundling is not wired to the new tree; the completion pass still runs on the frozen tree only. With the current 04:00 UTC dispatch and a 120-minute lead, fixtures kicking off before ~06:00 UTC are refused by design; widening that is a dispatch-time decision the owner controls (no schedule was changed).
5. **Verification (local venv, Python 3.11; CI is 3.13 — divergence reportable):** bare `.venv/bin/pytest` → **1168 passed**, `pyflakes` 0 warnings on `src/` + `scripts/` + both new test files, `py_compile` ok on `scripts/*.py src/slumdog/*.py tests/*.py`, `git diff --check` ok. Frozen-track regression is explicitly pinned: standard payload/manifest carry no `track`, no `timing_contract`, no `kickoff_utc`, and no new input-digest keys.

---

**Last updated:** 2026-09-22 (UTC) — **NEAR-TERM RE-CAPTURE (DAILY REFRESH) LANDED: T+1/T+2 dates with a completed run are re-captured daily so late-publishing leagues (baseball/basketball/tennis/mma/esports — previously "target date missing from HTML" at the D+5 capture) still enter the shadow pipeline. Owner-authorized 2026-09-22 as measurability/evidence-generation only: no sport weighting, no threshold/rule changes, frozen baselines untouched. Append-only everywhere: refresh picks relocate into the original run dir as `selections_delta_<stamp>.json` + `.manifest.json` (+ `.sha256` markers), evaluator `--exclude-events` freezes the original considered set visibly (`REFRESH_EXCLUDED_PREVIOUSLY_SELECTED` pool entries, refresh-mode-only manifest/payload keys — normal runs byte-stable, digests unchanged), refresh runs get their own run_id via the exclusion declaration entering the input digest, and `settlement_delta_<stamp>.json` (+ markers) grades deltas at D+1 with the same one-shot posture (idempotent per stamp, NOT_DUE guard, fail-closed marker verification, per-delta isolation). Batch order now: D+1 settlement → completion → delta settlement → refresh (T+1..2) → forward capture; flags `--skip-refresh` / `--refresh-days {1,2,3}` (default 2); batch receipt gains `refresh[]` + `delta_settlement[]` + summary keys. Gates (local venv, Python 3.11; CI 3.13 divergence stands): `py_compile` OK (4 changed files), pyflakes 0 warnings in `src/`+`scripts/`, `git diff --check` OK, bare `.venv/bin/pytest` → `1052 passed in 57.19s` (incl. 31 new tests: 5 evaluator refresh-exclusion, 9 settle-delta, 15 batch refresh, 2 collector force/receipt-name). Workflow file untouched (owner-only surface); the default-on refresh runs in the next dispatch with no workflow change required.**

**Last updated:** 2026-09-21 (UTC, second entry) — **#40 INCIDENT FULLY CLOSED: Depth Build #41 (`workflow_dispatch` on `7c3b2b3`, run `35593148939`) finished `success` at 14:08Z — all 14 jobs green, incl. Census `Run pytest` (the previously-failing completion CLI test passes on CI 3.13 with the clock-derived fixture) and the depth-sweep RAN unskipped (today's board census recovered); Research ran 11:18→14:08 with every step success (the `slumdog-research` artifact could not be downloaded from the sandbox — Azure signed-blob EOF, same as #33 — get `research_2026-09-21.md` from the run's step summary/artifact for the Sep-22 review).** Prior banner follows for history:

**Last updated:** 2026-09-21 (UTC) — **#40 CENSUS RED FIXED: the completion-pass CLI test had a calendar time bomb (fixture pinned to 2026-09-05 vs the real-clock 14-day retry window; first full-suite Monday after the fixture aged out). Production behavior was correct (RETRY_WINDOW_EXPIRED, clean exit 0 out of window); the test now builds its fixture date from today's clock, and a companion CLI test pins the expired-window path. 1015 passed locally (Py 3.11; CI 3.13 divergence stands).** Prior banner follows for history:

**Last updated:** 2026-09-14 (UTC, evening session) — **COMPLETION PASS FULLY LANDED: owner workflow commit `864c56a` (glob persistence, verified byte-identical to the validated replacement) and the AGENTS.md waiver-extension bullet (agent commit after owner instruction "proceed"; workflow files themselves remain owner-authored). The next forward-shadow dispatch runs the completion pass live and commits supplements/receipts end to end. Code: append-only supplements close UNSETTLED/UNRESOLVED rows within a 14-day window; decided grades immutable; 972 passed locally (Py 3.11), 0 pyflakes in src/scripts.**

1. **Why:** D+1 settlement runs once at 04:00 UTC, before evening US fixtures post; `find_settleable_run` skips any run already holding `settlement.json`, so 86 UNSETTLED + 2 UNRESOLVED rows on settled dates 09-08…09-13 (per-date UNSETTLED 1/0/9/5/41/30; the 2 UNRESOLVED are VOID rank-4+ football matches on 09-12) were frozen forever. The full in-window queue on 2026-09-14 is 137 rows across 9 runs (the extra 49 are on 09-02/05/06/07: 16 void matches → terminal, 33 old no-match rows expected to age out; supplements do not touch errata, which restate *decided* grades in `data/reports/shadow/errata/`). Owner authorized the completion pass 2026-09-14: retry both open grades, upgrade UNRESOLVED only on clean fresh identity + result, never re-decide SUCCESS/FAILURE, append-only, per-date isolated, dry-run, fail-closed marker verification. Full design is in `docs/STATE.md` → "Settlement completion pass".
2. **What changed (source + tests; no workflow/config/evidence change):**
   - `src/slumdog/shadow_settle.py`: new completion instrument (`SUPPLEMENT_SCHEMA_VERSION`, 14-day `COMPLETION_RETRY_WINDOW_DAYS`, `CompletionResult`, `load_verified_settlement`, `load_settlement_supplements`, `write_settlement_supplement`, `complete_settlement`, CLI `--complete` / `--completion-window-days`, exit 2 on `COMPLETION_FAILED`); `fetch_settlement_capture` gained `receipt_name` and writes purpose-tagged `settlement_capture_receipt_completion_<stamp>.json` instead of overwriting the D+1 receipt; `grade_to_dict` is now the shared serializer for D+1 rows and supplement rows (shape can't drift). Statuses: NOT_DUE / NOTHING_PENDING / RETRY_WINDOW_EXPIRED / NO_NEW_RESOLUTIONS / SUPPLEMENT_WRITTEN / COMPLETION_FAILED; terminal-UNRESOLVED reasons: `void_no_contest_or_cancelled`, `anomalous_draw_two_way_sport`, `underdog_identity_unavailable`, `ungradeable_result`.
   - `scripts/forward_shadow_batch.py`: `find_completable_runs` (D+1…14-day window, hash-verified, oldest first, malformed runs skipped), `run_completion_for_date` / `run_completion_backlog` (never raise; `--dry-run`; isolated per date; 62s pacing), wired into `main()` AFTER the D+1 backlog and BEFORE the forward pass (both behind `--skip-settlement`; counts in `forward_batch_receipt.json`; `--completion-window-days` passthrough). A date first-settled in the same invocation is skipped (`skip_dates`) so it isn't pointlessly re-fetched minutes later at the same 04:00 hour; it re-enters at D+2.
   - Tests: +36 (16 `tests/test_shadow_settle.py`, 20 `tests/test_forward_shadow_batch.py`).
3. **WORKFLOW PASTE DONE — owner commit `864c56a` (2026-09-14 12:14 UTC), verified byte-identical to the validated proposed replacement** (`git fetch && git show origin/main:.github/workflows/forward_shadow.yml` diffed against the proposed file; YAML parses; pinned SHAs `checkout@3d3c42e5…`, `setup-python@5fda3b95…`, `upload-artifact@043fb46d…a0a` unchanged; trigger stays `workflow_dispatch` only — the daily ~04:00 UTC run is owner/externally dispatched, there has never been a `schedule:` in this file). The agent never edited the workflow; the commit is owner-authored.
   - **AGENTS.md waiver bullet: done by the agent same day** (see sub-bullet below) — nothing remains owner-pasted for this feature.
   - Historical record — the two workflow edits now live on `main` in `.github/workflows/forward_shadow.yml` (the step label also gained "complete open settlement rows (append-only supplements)"):
     1. shadow `find` line gained `-o -name 'settlement_supplement_*.json' -o -name 'settlement_supplement_*.sha256'` inside the existing `\( … \)` group;
     2. evidence `find` line became `find data/settlement_evidence -type f \( -name 'settlement_capture_receipt.json' -o -name 'settlement_capture_receipt_completion_*.json' \) 2>/dev/null | xargs -r git add -f`.
   - **AGENTS.md waiver — DONE (agent docs commit on 2026-09-14 after the
     owner asked "how come you didn't do AGENTS.md yourself? proceed"):** the
     bullet mirroring the 2026-09-08 erratum waiver was appended under
     Filesystem Separation by the agent — workflow files remain owner-only,
     but AGENTS.md is not a workflow file and agent sessions have edited it
     before (e.g. `64d113e`). It records `settlement_supplement_*` +
     completion receipts as append-only small evidence, the 14-day retry
     window, immutable `settlement.json`/decided grades, fail-closed marker
     verification, and raw-body exclusion.
   - With the workflow globs live, the next dispatch COMMITS supplements and completion receipts. The oldest pending date (09-08) is last eligible at the 2026-09-22 dispatch; without the (now-pasted) globs it would have relied on the 30-day run artifact only.
4. **Verification (verified-locally this session, Python 3.11.2 venv; CI is 3.13 — divergence reportable):** bare `.venv/bin/pytest` → **972 passed in 51.24s, exit 0** (baseline 936 + 36 new; never pass `-q`, `pyproject addopts=-q` makes it `-qq`); `python -m py_compile scripts/*.py src/slumdog/*.py tests/*.py` → ok; `pyflakes src scripts` on the two changed files → **0 warnings**; `git diff --check` → ok. The new code's network path is not exercised in tests (offline/override seams are, including a monkeypatched fetch-boom on window expiry); the first live completion dispatches should be spot-checked via the batch receipt summary keys `settlement_completion_*`.
5. **Depth Build #33 (run `34831035940`) — FULLY GREEN, run conclusion `success` 2026-09-14 12:51:11Z (verified-via-API 13:02Z, owner confirmed).** All **14 jobs `success`**: Census (pytest + unskipped depth-sweep recovering the board census), all 11 History matrix, Aggregate, and **Research** (10:03:34Z→12:51:10Z, 2h48m; all 13 steps `success`, incl. "Run research gate (model cards + ablations)" and "Publish research summary"). The earlier "Research in_progress" caveat in the 2026-09-14 later-session entry below is now resolved — no jobs were skipped/failed; #33 is a complete recovery from #32.
6. Delivery mechanics unchanged from the prior entry: commit on `arena/01a09f64-slumdog`, fast-forward `main` (`git push origin HEAD:main`, retry via the refs PATCH API with `force=false` if refused), delete the session branch, report the resulting main SHA; never `--force`, no PR/merge commit. This changeset is code+tests+docs (AGENTS.md/HANDOFF.md/docs/STATE.md), training remains FROZEN, production NOT AUTHORIZED, no workflow schedules/gates/`timeout-minutes: 350` touched, no committed evidence or `config/research_baselines.json` modified.

---

**Last updated:** 2026-09-14 (UTC, later session) — **DEPTH BUILD #33 CENSUS GREEN IN CI: THE 2026-09-14 BOARD CENSUS WAS RECOVERED; THE PROPOSED PR-TRIGGERED VERIFY WORKFLOW WAS REJECTED BY THE OWNER — DO NOT RE-PROPOSE; DOCS-ONLY CHANGE ON TOP OF `main` @ `523190e9`.**

1. **Run #33 — Census green, census recovered (verified-in-CI).** #33 = run id `34831035940`, `workflow_dispatch` on `main` @ `523190e9` (merge with parents `33a4baf` + fix commit `6f9df1645` "fix: unbreak the four Census failures on main (two stale pins, one stale scope)"). Commands: `gh run view 34831035940 --json status,conclusion,jobs` and `gh api repos/6ixtyn9-sudo/Slumdog/actions/jobs/103934191407`. The Census job's **all 12 steps ended `success`**: `Run pytest` (Python 3.13, bare `pytest` exactly as `pipeline.yml` invokes it) ran 10:02:03Z→10:04:03Z, and the depth-sweep step then **RAN unskipped**, 10:04:04Z→10:19:09Z, with census summary, `slumdog-census` artifact and detail-page cache save all green. That unskipped sweep **is** the recovered 2026-09-14 board census — failed #32's pytest step stopped the job so its sweep was skipped and the date initially had no census. All 11 `history` matrix jobs ended `success`. The Research job was still `in_progress` when this handoff was written (~2h53m, parallel; per owner instruction not waited for). Retrieval caveat, no gap-filling: the literal "936 passed" CI log line could **not** be downloaded from this sandbox (the signed Azure blob log endpoint returned EOF on four attempts, and the CLI withholds per-job logs until the whole run completes); the Actions API step conclusion `success` is the authoritative gate, and pytest exits non-zero on any failure while the sweep only starts after pytest passes.
2. **Task B dropped by owner decision — no `.github/workflows/verify.yml`, ever re-created without owner sign-off.** The handoff into this session specified a fast `pull_request` verify workflow; after discussion the owner rejected it. Rationale (full text in `docs/STATE.md` → Verification, "No branch/PR verification workflow"): (a) the weekly Monday census already runs the full suite as its first step and that gate caught all four #32 failures; (b) workflow files are owner-hand-authored and intentionally kept out of agent delivery (the Arena App token has historically also been refused `.github/workflows/*` writes); (c) the #32 root cause was tests pinned to dates/evidence that drift — two failures were expired date pins green when written, which a pre-merge CI run could not have caught; (d) bounded *data* impact: a red Monday loses at most one weekly board census, noticed that morning and recovered by a same-day manual `workflow_dispatch` — #33 did exactly this and had the census back by 10:19Z. The mitigation chosen instead is one load-bearing sentence in the `AGENTS.md` pre-push checklist (the local suite is the real gate because a red suite on `main` skips the sweep). **No workflow file was created or modified; `pipeline.yml` schedules, cron strings, `if:` gates and `timeout-minutes: 350` are untouched.**
   - **(e) Cost-rationale CORRECTION (owner, 2026-09-14, same day) — do not re-argue from runner cost.** The rejection rests solely on (a) workflow files being owner hand-authored and (b) the weekly Monday census already gating on `pytest` as its first step. An earlier note's claim that the verify workflow was not worth "~3 hours of runner/relay cost" is **void**: the repo is **public** (`gh api repos/6ixtyn9-sudo/Slumdog -q .visibility` → `public`, verified), GitHub Actions minutes are free and unlimited for public repos, and `verify.yml` would do no network/relay work anyway. Point (d) bounds the *data* risk (one recoverable census), it is not a runner-cost argument. Decision unchanged; recorded reasoning corrected in both this file and `docs/STATE.md`.
3. **Local verification (verified-locally; Python 3.11.2 venv from `python3 -m venv .venv && .venv/bin/pip install -e '.[dev]'`):** bare `pytest` (the CI invocation) → **936 passed in 55.75s, exit 0**, 936 collected across 55 test files (collection cross-check; CI runs the same suite green on Python 3.13). `python -m py_compile scripts/*.py src/slumdog/*.py tests/*.py` → ok. `pyflakes scripts src/slumdog tests` → exactly the **13 pre-existing expected warnings**, all in `tests/test_dataset_conflict_census.py`, `tests/test_dataset_final_integrity.py`, `tests/test_dataset_hardening.py`, `tests/test_research_incremental_builder.py`; **zero** warnings under `scripts/` or `src/`. `git diff --check` → ok. *Tooling note for the next agent:* `pyproject.toml` already sets `addopts = "-q"` — passing `-q` again yields `-qq`, which suppresses the final "936 passed" summary line (and makes `--collect-only` print per-file grouped counts instead of node ids); run bare `.venv/bin/pytest` and let `addopts` do the work.
4. **Changed files (docs-only):** `AGENTS.md` (pre-push checklist sentence), `docs/STATE.md` (CI verification of `6f9df1645` on #33, census recovery, no-verify-workflow decision with rationale), this file. No source, test, config (`config/research_baselines.json` hash still `666dabe7…`), committed evidence, or workflow change; training remains FROZEN; production NOT AUTHORIZED.
5. **Full-history secret scan — CLEAN (2026-09-14; the repo is public, so all committed history is world-readable).** The owner's current-tree scan at `523190e` found nothing; this session closed the history gap after `git fetch --unshallow` (192 commits, 98,277 lines of full diff): credential patterns (`ghp_`/`github_pat_`/`AKIA`/`sk-`/`-----BEGIN … PRIVATE KEY`) → **0 matches**, grep rc=1; Azure blob/SAS strings (`blob.core.windows.net`, `sig=`) → 0 across all history and tracked HEAD; no tracked `.env`/`.pem`/`.key`/`id_rsa`/credential filenames; the only credential-keyword hit is a `forebet.py` docstring ("no secrets"). The SAS URL from chat was never committed and is expired. Commands and exact API findings are in `docs/STATE.md` → Verification ("Credential/secret scan"). **Dependabot alerts are DISABLED** (API: explicit 403 message); secret-scanning/push-protection enablement state could not be read with the Arena token (403/404 — state unknown, not assumed off). These are owner-clicked settings (free for public repos): Settings → Code security → Secret scanning + Push protection; enable Dependabot alerts/updates (deps carry open lower bounds: `numpy>=1.26`, `scikit-learn>=1.4`, `beautifulsoup4>=4.12` — see `pyproject.toml`). Before enabling **push protection**, check the workflow `GITHUB_TOKEN` auto-commits (`forward_shadow.yml` with `contents: write`): push protection applied to those pushes could block captured-content commits on a false positive — confirm the bot identity is exempt or risk a failed daily job. This session changed no security settings.
6. **Delivery mechanics used (the session was pinned to `arena/01a09f64-slumdog`):** commit on the session branch as the docs-only child of `523190e9`, then fast-forward `main` with `git push origin HEAD:main`; if the Arena delivery token refuses the direct push, fast-forward the ref through the API without force (`gh api -X PATCH repos/6ixtyn9-sudo/Slumdog/git/refs/heads/main -f sha=<child> -F force=false`); delete the session branch afterwards (`--delete`, or `gh api -X DELETE .../git/refs/heads/<branch>`). Never `--force`; no PR. Result and resulting main SHA are reported in the session summary; if both paths were refused, the commit stays on the session branch and is handed to the owner to fast-forward — it is never claimed as landed without a command proving it.

---

**Last updated:** 2026-09-14 (UTC) — **THE MONDAY CENSUS RUN'S FOUR FAILURES ARE FIXED, AND TWO OF THEM WERE THE TEST SUITE TELLING THE TRUTH ABOUT `main` RATHER THAN ABOUT THE CODE.** `main` @ `33a4baf` (Census — current board, all sports, run 34818816638) went **4 failed / 913 passed**; it is **936 passed, 0 failed, 0 deselected, 0 skipped** now, under both `pytest` and `python -m pytest`. Three separate causes, no common root, and **no production-code change was needed for any of them** — every fix is in the tests or in the erratum instrument's scope rule:

1. **A pin, not a bug (`test_real_evaluator_does_not_block_on_seeded_manifests`).** The end-to-end history-selection regression pinned its target date to `2026-09-10`. The evaluator refuses to commit once the clock passes `target_date 00:00:00 UTC − 24h`, so from 2026-09-09T00:00Z onward the test blocked with `DECISION_COMMITTED_AT_AFTER_SAFE_CUTOFF` — for a *timing* reason, not the `HISTORY_LOAD_FAILED` it exists to catch. The date now travels with the clock (`_clock_safe_target_date`, +3 days), so the test measures history selection forever instead of expiring.
2. **A stale vocabulary, not a refinement (`test_status_label_was_not_refined[2026-09-16]`).** `considered_status` is one of the six fields `pool_for_digest` projects, so its label set is frozen. On 2026-09-16 the first row outside that set appeared: `IDENTITY_INELIGIBLE:EQUAL_PROBABILITY`, one row — `football:2548491`, AB Gladsaxe vs Sonderjyske at 0.34/0.34. It is emitted by the **pre-existing** identity gate (the review doc records it as producible with zero occurrences in the evidence the pin was written against), and an exact equal-probability row must be excluded explicitly rather than assigned silently, so **the label is right and the pin was incomplete**. The reason vocabulary is now pinned alongside the label, a new test re-derives every composite label from `input_provenance.capture_record_tuples` (falsifiable, not trusted), and a negative control proves the check still rejects both an unpinned reason and a refined `R2_INELIGIBLE:*` label. `FEATURE_INCOMPLETE_OR_R2_INELIGIBLE` is untouched.
3. **`test_every_settled_date_is_covered` + `test_skip_existing_…` — the fix caught up with the instrument.** PR #18 merged 2026-09-07T15:43Z, so the six settlements published since (2026-09-08 … 2026-09-13) carry `underdog_index ∈ {1,2}` and rank-4+ SUCCESSes: **they are not defect-affected**. Demanding an erratum per *settled* date — correct while every settlement was defective — now asserts "a rank-4+ SUCCESS was unreachable" about evidence where one is recorded, and breaks the per-erratum `committed["successes"] == 0` invariant. The generator now decides scope from the evidence (any rank-4+ row still on the draw sentinel); out-of-scope runs are still visited, hash-verified and *reported* as `not defect-affected`, never silently skipped. Coverage is pinned from both sides: every defect-affected date has an erratum, and every date without one is re-derived as clean from its own artifact. **Corrected totals stay 283 / 855 = 33.1%** and no longer move as new dates settle.

**Nothing was laundered here.** No committed evidence, config, or workflow file was modified; no erratum was added, removed, or rewritten; `settlement.json` + `.sha256` markers remain byte-identical; the frozen baseline config hash is unchanged at `666dabe7…`. No grading rule, threshold, or R2 feature was touched, and the corrected numbers still must not be used to justify any rule change.

**Branch:** `arena/01a09f1f-slumdog` from `main` @ `33a4bafae5ceda4d1e693279a83d7fbdb7407674` ("chore: shadow evidence (run 34804470799)").
**Tests:** 936 passed, 0 failed, 0 deselected, 0 skipped under **both** invocations (bare `pytest`, as `.github/workflows/pipeline.yml` runs it, and `python -m pytest`); 917 before (+19). pyflakes clean on every changed file (the one pre-existing warning in `tests/test_forward_shadow_batch.py` was cleaned while fixing its date pin); 13 pre-existing warnings remain in untouched `tests/test_dataset_*` and `test_research_incremental_builder`. `py_compile` ok, `git diff --check` ok.
**Changed files:** `tests/test_forward_shadow_batch.py`, `tests/test_r2_exclusion_reason.py`, `tests/test_rank4_settlement_erratum.py`, `scripts/rank4_settlement_erratum.py`, `docs/STATE.md`, `docs/ADVERSARIAL_REVIEW_RANK4_SETTLEMENT_FINDINGS.md` (new §8 addendum), this file.

---

Prior state (2026-09-08) — **THE PRE-GRADE EXCLUSION BLACK BOX IS OPEN: why 56–82% of every considered pool was discarded is now recorded per row.** `FEATURE_INCOMPLETE_OR_R2_INELIGIBLE` merged five distinct causes behind one label — and the label's own `OR` admitted it. `is_r2_eligible` computed the distinction and returned a bare `bool`; the excluded pool rows kept only six keys, so the cause was **unrecoverable from committed evidence**. That destroyed the only population able to answer a live question: `forebet_probability_gap <= 0.2` is the R2 gate, and the rows that *failed* it are exactly what is needed to judge whether 0.2 is right. New `r2_ineligibility_reason()` decomposes all five failure points with actual values (`missing_fields` named individually; every failed threshold carrying `feature`/`op`/`threshold`/**observed**, so gap 0.21 and gap 0.6 no longer collapse to the same `False`), attaches `r2_exclusion` to every pool row, and aggregates a new top-level `r2_exclusion_breakdown`. Digest safety is **proven on the four real committed settled manifests**, not assumed: injecting the key into every row leaves `decision_digest` byte-identical, with a negative control (touching `decision_accounting` *does* change it) so the proof cannot pass vacuously. A finding changed the design — driving the real evaluate path showed `prior_games` arriving as `0.0` with missingness flag `0`, i.e. *absent history*, not a strict cutoff, so the flags are recorded to keep the two distinguishable. **`is_r2_eligible` untouched, all thresholds unchanged, frozen config hash still `666dabe7…`.** Full suite **887 passed, 0 failed, 0 deselected, 0 skipped** (95 new). See `## R2 Exclusion Reasons` below.

Prior state (2026-09-08, earlier this session): **BOTH OPEN BLOCKERS CLEARED, AND A FOURTH DEFECTIVE ARTIFACT FOUND STILL BEING PUBLISHED BY `main`.** (1) The `AGENTS.md` scoped-commit waiver now covers `data/reports/shadow/errata/**` (extended, not relocated). (2) `fe031ae5…` is **fully reconciled** — `git fetch --unshallow` recovered 168 commits and the hash is the declaration as committed at `ebc4e6fc`/`303b09f6`; the pin was correct at birth and went stale at `4c63575e`, four commits before current. The alignment to `20f9a3a3…` is now *proven*, and the HOLD on that part of PR #18 is lifted. (3) Chasing the "09-07 settlement gap" found something worse: it is **not a gap** — the D+1 automation settled 2026-09-07 on `main` @ `1ee80d4` at 2026-09-08T04:00:49Z using the **unfixed** code, so all 37 rank-4+ rows carry `underdog_index == 0` and 35 are fabricated FAILUREs. A fourth erratum is published (true rate 13/35 = 37.1% vs committed 0/35); totals are now **283/855 = 33.1%**, Wilson 95% [0.300, 0.363]. Up to **six more** defective artifacts (09-08 … 09-13) are already scheduled on the same automation until PR #18 merges. The generator gained `--skip-existing` so new dates append without touching published evidence. Full suite **792 passed, 0 failed, 0 deselected, 0 skipped**.
...
**Tests:** 887 passed, 0 failed, 0 deselected, 0 skipped under **both** `pytest` (bare, as `.github/workflows/pipeline.yml` runs it) and `python -m pytest`. The bare invocation used to fail 39 tests in `tests/test_forward_shadow_batch.py` with `ModuleNotFoundError: No module named 'scripts'` — `pythonpath = ["src"]` never put the repo root on `sys.path` and `scripts/` has no `__init__.py`, so `python -m pytest` (which prepends the CWD) masked it. Fixed with `pythonpath = ["src", "."]`.
**Working tree:** committed and **pushed** on `arena/01a07b8b-slumdog`; local `HEAD` == remote tip == `bf2b9f3`. **PR #18 OPEN against `main`, `MERGEABLE`, NOT merged** (owner sign-off still required; `AGENTS.md` forbids merging without explicit authorization). Commit chain: `8ddb2da` (review doc) → `2136535` (rank-4+ fix + erratum + pin) → `03006f3` (discard audit: odds_X, settled_context, per_rank_band) → `b3441e5` (retain facets by default; invariant-10 bar moved to the feature layer) → `0087454` (handoff PR-state refresh) → `e159140` (merge `origin/main` @ `1ee80d4`, bringing the 09-07 settlement evidence local) → `31444d8` (errata waiver scope resolved) → `1655898` (`fe031ae5` reconciled; both blockers closed) → `eaa1564` (4th defective artifact, 09-07; `--skip-existing`) → `bf2b9f3` (R2 exclusion-reason recovery). The earlier "push blocked / no GitHub credential path" note is **superseded**, but note the Arena `GH_TOKEN` is **intermittently invalid** — pushes on 2026-09-07 failed with `Bad credentials` / `no longer valid` and then succeeded on retry minutes later. On a push failure, retry and re-check `gh auth status` before concluding the connection is broken; ask the owner to reconnect GitHub in Arena only if it stays dead.

**Sandbox git caveat (verified 2026-09-07):** the Arena sandbox reset the local `arena/01a07b8b-slumdog` pointer back to the base commit `edd5ff2` while leaving the working tree intact, so all prior work appeared uncommitted and `git log` showed one commit. Recovered with `git fetch origin arena/01a07b8b-slumdog` + `git reset --mixed 2136535` after proving every "untracked" file was byte-identical to the committed version. **Check `git log` against the remote before assuming work is missing.**

Discard-audit changes (this increment): `src/slumdog/contracts.py` (`SettledEvent.odds_draw`), `src/slumdog/settlement.py` (`_draw_odds` helper + all four parser paths + American-format facets), `src/slumdog/dataset.py` (rehydration + `PROHIBITED_KEYS`), `src/slumdog/facets.py` (catalogue entry), `src/slumdog/shadow_contracts.py` (barred record field), `src/slumdog/shadow_settle.py` (`_settled_context`, `WITHHELD_FACET_KEYS` — empty, retention by default, `RANK_BANDS`/`per_rank_band`, conflict detection, `metadata_policy`), new `tests/test_discarded_settlement_metadata.py` (47 tests), `docs/STATE.md` + this file refreshed.

Prior increment (already in `2136535`): `src/slumdog/shadow_evaluator.py`, `src/slumdog/shadow_settle.py`, `scripts/synthetic_shadow_fixture.py` (stale declaration-hash pin), new `scripts/rank4_settlement_erratum.py`, new `tests/test_rank4_settlement_erratum.py`, extended `tests/test_shadow_settle.py` + `tests/test_shadow_evaluator.py` + `tests/test_synthetic_shadow_fixture.py`, new `docs/ADVERSARIAL_REVIEW_RANK4_SETTLEMENT_FINDINGS.md`, new `data/reports/shadow/errata/**`. **No committed evidence, config, or workflow file was modified in either increment** — `settlement.json` + `.sha256` remain byte-identical, and the frozen baseline config hash is unchanged.

## R2 Exclusion Reasons

Owner instruction: the exclusion data is "the same class as the kelly / best_odd_X
/ facets fixes — data computed then discarded", to be decomposed at *all five*
failure points with actual values ("missing field names; real threshold numbers
like gap=0.21 vs gap=0.6"), reusing the existing `pool_for_digest` fixed-key
projection rather than reinvestigating digest safety, and framed with a
`metadata_policy`-style governance note. Committed as `bf2b9f3`.

**The defect.** Between **56% and 82%** of every considered pool was excluded
before grading under one label: `FEATURE_INCOMPLETE_OR_R2_INELIGIBLE`. The `OR`
is in the name — the label admits it merges two different situations, and in
fact five distinct causes sat behind it. `is_r2_eligible` evaluated four
thresholds plus a missingness check, short-circuited, and returned a bare `bool`.
The excluded pool rows persisted only six keys. So the cause was not merely
unrecorded but **unrecoverable from committed evidence**: no re-analysis of the
artifacts could ever separate them.

**Why this discard cost more than the others.** `forebet_probability_gap <= 0.2`
is the R2 gate. The rows that *failed* it are precisely the population needed to
judge whether 0.2 is the right number — the distribution of rejected gaps is the
evidence. It was thrown away, so "is 0.2 too tight?" could only ever be answered
by opinion. **That question remains open and is not answered here.** What changed
is that it becomes answerable from future runs.

**What is now recorded.** `baseline_analyzer.r2_ineligibility_reason(features,
missingness)` returns `None` when the frozen rule admits a record, and otherwise:

| key | content |
| --- | --- |
| `primary_reason` | `MISSING_FEATURES` / `THRESHOLD_NOT_MET` / `BOTH` — a grouping label, never a substitute for the values |
| `missing_fields` | the specific absent feature names, individually |
| `failed_thresholds` | `feature`, `op`, `threshold` **and the observed value** |
| `observed_values` | all four features as seen, so the excluded distribution is reconstructable without re-running anything |
| `missingness_flags` | the companion flags, per the finding below |
| `rule` | `R2_CONSERVATIVE_FIXED_RULE` |

Every check is evaluated regardless of short-circuit order, so a row missing
`h2h_prior_games` *and* over the gap threshold reports `BOTH` — `is_r2_eligible`
would only ever have said `False`. `r2_exclusion` is attached to **every**
considered-pool row (`None` when eligible, or excluded for a non-R2 reason:
conflict, duplicate, malformed, timing) and aggregated into a new top-level
`r2_exclusion_breakdown` manifest section.

**A finding that changed the design.** Driving the real evaluate path with no
history showed `*_prior_games` arriving as `0.0` with missingness flag `0` — a
genuine zero meaning "nothing in the bounded last-5 window", not an imputed
value and not `None`. (Rate features derived from those zero games carry flag
`1`, which is coherent: 0 games is a fact, a win rate over 0 games is not.) The
consequence is that the dominant rejection will read as `THRESHOLD_NOT_MET` on
`prior_games >= 5` with `observed: 0.0`, which is semantically **absent data,
not a strict cutoff**. Without the flags those two cases are indistinguishable,
and a reader would conclude the rule was too tight when the truth is there was no
history at all. The flags are recorded for exactly that reason.

Values are stored **unrounded**. `0.6 - 0.2` lands as `0.39999999999999997`, and
a gap of `0.20000000000000004` fails `lte 0.2` while displaying as `0.2`.
Rounding would hide the boundary cases a cutoff review most needs to see.

**Digest safety — proven, not assumed.** `decision_digest` is computed over
`decision_provenance`, whose `considered_pool` is the six-field `pool_for_digest`
projection (`sport`, `event_id`, `event_date`, `considered_status`, `eligible`,
`rank_within_sport_day`). A new key on a pool row is projected away and cannot
reach the digest — the same mechanism the rank-4+ identity fix relied on, reused
per instruction rather than reinvestigated. Verified against the **four committed
settled manifests** (2026-09-02 / 09-05 / 09-06 / 09-07), not a synthetic case:

1. `canonical_sha256(decision_provenance) == decision_digest` reproduces;
2. projecting the manifest's full pool rows reproduces the committed digest input;
3. injecting `r2_exclusion` into every row leaves the digest **byte-identical**;
4. **negative control** — touching `decision_accounting` *does* change it, which
   is what proves (3) is sensitive rather than passing vacuously.

Because of (4), the aggregate deliberately lives in a new **top-level** manifest
section and *not* in `decision_accounting`, which is part of the digest payload
and would have broken reproducibility of every committed run.

**What was deliberately left alone.** `considered_status` still carries the
conflated label. Refining it would be cleaner, but it *is* a projected field, so
changing it would rewrite `decision_digest` and `run_id` for historical runs. The
per-row reason carries the distinction instead — widening what is recorded
without touching what decides.

**Governance.** This records; it decides nothing. `is_r2_eligible` is untouched
and remains the single frozen rule; all thresholds unchanged;
`config/research_baselines.json` unmodified (hash still `666dabe7…`);
`load_frozen_baseline_config`'s exact-R2-rule-shape guard still passes, and
`R2_ELIGIBILITY_SPEC` is asserted equal to the declared config rule so code and
config cannot drift apart silently. `r2_exclusion_breakdown` carries an explicit
`policy` block: `purpose: recording_only`, `decides_nothing`,
`not_justification_for_tuning`, and a note stating the data is **NOT** a
justification for changing `gap <= 0.2` or any other R2 threshold — `anti_tuning`
prohibits result-driven amendments, and any threshold change requires a separate,
explicit, **owner-approved tuning decision**. Recording why a frozen rule
rejected a record and weakening that rule are different acts; this is the first
only.

**Files changed.** `src/slumdog/baseline_analyzer.py` (`r2_ineligibility_reason`,
`R2_ELIGIBILITY_SPEC`, `FROZEN_R2_RULE_NAME`, `_R2_COMPARISONS`, docstring on
`is_r2_eligible` marking it frozen); `src/slumdog/shadow_evaluator.py`
(`r2_exclusion` in both pool key sets with a digest-safety comment, wired through
3 decision-stage return paths and 6 pool-row construction sites,
`_summarise_r2_exclusions`, the new manifest section); new
`tests/test_r2_exclusion_reason.py` (95 tests). **No committed evidence or config
was modified.**

**Verification.** 95 new tests: rule/record agreement swept over a **480-point
boundary grid** across all four features rather than cherry-picked cases;
spec-vs-config equality; the five failure points separately visible; unrounded
values; the missingness-flag semantics; digest identity on real settled evidence
with negative control; end-to-end wiring through the production
`_evaluate_for_decision_stage`; manifest shape via the synthetic fixture;
aggregate tallies; and the governance assertions. Full suite **887 passed, 0
failed, 0 deselected, 0 skipped**, pyflakes clean. The existing schema pins
(symmetric-difference and subset assertions on the pool key sets,
`tests/test_shadow_evaluator.py`) were updated for the new key and are green.

**Settlement state (item 6, chased and closed).** All four settled dates
(2026-09-02, 09-05, 09-06, 09-07) now have published errata and are corrected.
**09-08 … 09-14 have no `settlement.json` at all** — they are unsettled, awaiting
real match results, not missing an erratum. The forecast stands: 09-08 … 09-13
will settle defectively on `main`'s unfixed automation unless PR #18 merges
first, and `--skip-existing` lets the generator append those dates without
touching published evidence.

## Discarded Settlement Metadata

Owner instruction: "action everything that you would action now", with two
specific reminders — "don't forget odds_X / there's odds_X" and "look at
everything that's being discarded". A full audit of what each stage *collects*
versus what it *keeps* found the settlement path discarding data it had already
parsed.

**Discard 1 — the draw price, lost twice.** `parsers._participant_odds` returns
`(odds_1, odds_2, values)`; on a three-way board it evaluates `parsed[1]` (the
draw price) and returns `parsed[0], parsed[2], values`, so the draw price
survived only as raw text inside `values` — which both call sites discarded via
`_`. Separately, `settlement.parse_football_settled` read only `best_odd_1` and
`best_odd_2`, never `best_odd_X`. The asymmetry that hid this: the **pre-event**
path *does* capture it (`parsers.py` → `facets["odds_draw"]` from `best_odd_X`,
and `best_odd_X_am` → `odds_draw_am`), so the number existed before kickoff and
disappeared after the match.

Fix: `SettledEvent.odds_draw`, populated on every settlement parser — the JSON
football path, the HTML path, and the shared `**base` dict feeding
mma/esoccer/cricket — plus `dataset.py` rehydration. `_draw_odds` deliberately
mirrors `_participant_odds`' branch conditions so the two can never disagree
about which cell is which: for draw-capable sports whose board leaves the draw
cell blank or dashed (handball, cricket — verified behaviour) it returns `None`
rather than reading `parsed[1]` positionally and reporting the **away** price as
a draw. Missing stays missing; nothing is zero-filled.

**Discard 2 — `SettledEvent.facets` built then dropped.**
`parse_football_settled` assembles a rich facet dict (stadium, weather,
goalsavg, Round, league positions, `move_1/move_X/move_2` odds movement, cup
flags, HT/ET/penalty scores, `trend_en`) and passes it to `SettledEvent` — but
`write_settlement_artifact` serialised a fixed list of grade fields, so all of it
was lost at the last step. Likewise the pre-event `EventSnapshot.facets` are
consumed into 17 derived features and then not persisted (`shadow_evaluator.py`
has no reference to `.facets` at all). Each grade row now carries a curated
`settled_context`.

**Governance binding.** Everything recovered is metadata, not signal, and the
artifact now says so itself: a `metadata_policy` block asserts
`odds_used_in_grading: false`, `odds_used_as_model_features: false`,
`odds_gate_candidates: false`, `missing_odds_lower_confidence: false`, and names
AGENTS.md invariants 8–11 — so the policy travels with the numbers instead of
living only in a doc a future agent may not read. `odds_draw` was added to
`dataset.PROHIBITED_KEYS` and `shadow_contracts._FORBIDDEN_RECORD_FIELDS` and
declared in the facet catalogue as `PRE_EVENT`, barring it as a feature at every
layer it could otherwise enter.

**Retention is the default; `kelly` is kept.** An earlier draft of this change
withheld Forebet's published Kelly fraction from the artifact on invariant-10
grounds. The owner overruled that on 2026-09-07: collected data is not to be
discarded, because the mission is to predict the underdog and any tool that gets
us there should be retained. Facets are now persisted in full and
`WITHHELD_FACET_KEYS` is an **empty** governance denylist.

The invariant-10 bar moved rather than vanished — **recording a number and
staking on it are different acts.** `kelly` was added to
`dataset.PROHIBITED_KEYS` alongside `overround`, `value_edge` and `ROI`, so the
retained datum can never become a model feature. That is where the guard against
EV / de-vigging / Kelly drift actually belongs: at the point features are built,
not at the point evidence is recorded. The withholding mechanism was kept rather
than deleted, so any future exclusion is a deliberate act named per row under
`facets_withheld` instead of a silent drop.

**Two further fixes actioned from the same review:**

- `per_rank_band` — `per_rank` is kept as the audit view, but on a ~500-event
  sport-day nearly every rank holds exactly one row (2026-09-05: 507 cells), so a
  per-rank hit rate there is n=1 noise formatted as a statistic. Six bands
  (`1`, `2-3`, `4-10`, `11-25`, `26-50`, `51+`) pool the same rows and expose
  `n` per cell. `per_rank` keys now sort numerically, not lexicographically.
- Conflicting capture records — `prob_lookup[key] = (…)` was last-write-wins, so
  two `capture_record_tuples` rows sharing a `sport:event_id:event_date` key with
  **different** pre-event probabilities would grade using whichever arrived last,
  i.e. capture ordering would decide the grade. Such keys are now detected and
  the identity refused with provenance `unavailable_conflicting_capture` →
  `UNRESOLVED`. Identical duplicates — the case actually present in the 09-12
  data — still resolve normally, and a regression test pins both behaviours.

**Flagged, not changed:** `american_football.py`, `baseball.py`, `basketball.py`,
`cricket.py` and `esports.py` each already export `calculate_overround_*` and
`devig_probabilities_*`. De-vigging is named in invariant 10. These predate this
work, sit outside the shadow/certification lineage, and were left untouched —
reported to the owner rather than amended unilaterally.

**Scope discipline:** no grading rule, threshold, ranking, or config changed.
`config/research_baselines.json` remains `666dabe7…` and
`config/shadow_evaluator.json` remains `20f9a3a3…`. The committed `settlement.json`
files and their `.sha256` markers are byte-identical; this change affects only
artifacts written **from now on**. No corrected or newly-recovered number is used
to justify any rule change.
...
Settlement automation proven end-to-end on 2026-09-06: the first real dispatch settled 2026-09-02 and 2026-09-05 (artifacts + capture receipts on `main`); remaining dates settle automatically as their D+1 arrives.

Pending: (1) durable storage beyond 30d artifacts.
...
## Milestone 7E — Automated D+1 Settlement (MERGED VIA PR #16 / PROVEN IN PRODUCTION / NO NEW WORKFLOW FILE, 2026-09-06)
...
**Honest status:** implemented YES / unit-tested YES (31 new tests across `tests/test_forward_shadow_batch.py` and `tests/test_shadow_settle.py` + 5 history-selection regression tests in `972b79a`; full suite green) / manual dry-run smoke-tested YES / executed on GitHub YES (multiple production dispatches on 2026-09-06) / real automated settlement YES for 2026-09-02 and 2026-09-05 (the only dates D+1-eligible as of the first dispatch; remaining dates settle as their D+1 arrives). Merged to `main` via PR #16.
...
## Milestone 7F — Uncapped cohort recording + Milestone 7D retirement (IMPLEMENTED ON BRANCH / PR PENDING OWNER SIGN-OFF, 2026-09-07)
...
- `tests/test_shadow_evaluator.py`: `test_ranks_4_plus_beyond_top3_in_considered_pool_only` → `test_uncapped_cohort_records_every_eligible_ranked_event` (8 eligible events → 1 primary + 7 cohort in `selections[]`, zero beyond-top3 anywhere, accounting 1/7/0).

**Change 2 — Milestone 7D retired.** Root cause of the 12 long-deselected test failures, verified against real `main` history: `.github/workflows/shadow_bundle_cloud_backup.yml` was merged via PR #13 (`b086eae`, 2026-08-30), then deliberately deleted from `main` by the owner on 2026-09-03 (`5641111`); `tests/test_cloud_backup_workflow.py` (12 tests hard-requiring that file, last touched `c35a8d1`) was never removed, so it failed on `main` from 2026-09-03 onward while being masked by `--deselect` in every subsequent session. Owner decision 2026-09-07: retire, not restore. The test file is deleted; `scripts/synthetic_shadow_fixture.py` + its 9 tests are self-contained and unaffected; `docs/MILESTONE7D_CLOUD_BACKUP.md` kept as history. The `--deselect` workaround should no longer appear anywhere.

**Verification (this session, on the branch):** `python -m pytest` → **718 passed, 0 failed, 0 deselected, 0 skipped**; pyflakes/py_compile/`git diff --check` clean on touched files. Post-merge acceptance check: the next `forward_shadow.yml` dispatch's `manifest.json` must show, for at least one sport-day, `top3_cohort_selected` reflecting every eligible-ranked event with `eligible_ranked_beyond_top3 == 0` — unit tests alone don't prove the production path fired.
...
**Version-suffix cleanup (owner directive 2026-09-07, same PR):** the system carries no `_v1`/`_v2` naming anymore: config files renamed `config/shadow_evaluator.json` and `config/research_baselines.json` (content bytes unchanged, so canonical hashes `fe031ae5…`/`666dabe7…` are unchanged and still pinned); live artifact schema tags de-suffixed (`shadow_evaluator`, `shadow_settlement`, `slumdog_shadow_bundle`, `forward_shadow_batch`); history-loader `_v2_filter_one`/`V2_INVALID` identifiers renamed to ledger-validity names. The dataset-contract version strings were **also** de-suffixed after explicit owner sign-off (2026-09-07): `price-free-minimal-2026-08-24` (feature), `price-free` (label), `canonical` (canonical repr), `price-free-incremental-valid-history` (research feature), `slumdog-research-input` (research digest domain), `assessment_version` defaults. Correction to the handoff's risk note: there WAS exactly one hardcoded golden pin depending on these bytes — `test_shared_feature_golden_regression` (`GOLDEN_SHARED_FEATURE_DIGEST`/`GOLDEN_CANONICAL_BYTE_COUNT` over the serialized `build_price_free_examples` output). It was re-anchored once, in the same commit, with a provenance comment: digest `1a97cb81…`→`ff8acd0b…`, bytes 21430→21340, an exact −90-byte delta = 15 examples × 6 removed chars, proving the rename itself was the only byte change. All other digest tests compute expected values from the same production constants on both sides and follow the rename automatically. The workflow step label `timing-v1` was renamed by the owner directly (`610e4b7`). Pre-rename artifacts on `main` keep their old tags forever (immutable evidence with sha256 markers); consequence: runs/bundles created before this cleanup cannot be re-bundled/re-verified with the renamed constants (they were verified at creation; no live path re-processes them). **The one permanent v1-shaped exception (intentional, not an oversight):** the backward-compat acceptance list in `_validate_settled_dict` (`dataset.py`) still accepts `schema_version` values `"v1"`, `"canonical-v1"`, `"price-free-v1"` — that list exists to keep *already-committed historical ledger files* readable, not to describe the current contract; do not extend or remove it.
...
- **Hardened module:** `src/slumdog/dataset.py` (~1400 lines, 4F)
  - `FEATURE_CONTRACT_VERSION = "price-free-minimal-2026-08-24"`, `LABEL_CONTRACT_VERSION = "price-free"`
...
- **Modules:** `src/slumdog/research_dataset.py` (orchestration, streaming emitter, bounded views, safe finalization) + new `src/slumdog/research_builder.py` (research-mode eligibility, duplicate normalization, incremental builder core). Both research-only; neither is imported by production pipeline modules (pipeline, training, backfill, depth_sweep, research, forebet, cli).
- **`build_price_free_examples` (strict) is unchanged** and serves as the byte-level reference for equivalence tests.
- **Research feature contract:** `price-free-incremental-valid-history` on every example/sample/receipt; label contract unchanged. The only semantic change vs strict mode is history membership: `research_history_eligible` (coherent disposition/winner, known sport, distinct participants) replaces the implicit legacy `HistoryIndex` filter — intentional divergences: unknown-sport rows, `NO_CONTEST` aliases, and incoherent rows (e.g. `SETTLED_CUP` winner-0) no longer feed history.
...
- **Active branch:** `arena/01a07b8b-slumdog` — adversarial review, rank-4+ settlement remediation, append-only erratum, and the discard audit (base `main` @ `edd5ff2`; head `b3441e5`, pushed).
