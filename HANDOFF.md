...
**Last updated:** 2026-09-14 (UTC, evening session) — **SETTLEMENT COMPLETION PASS BUILT, TESTED, COMMITTED-READY (owner decision `build_include_unresolved`): append-only supplements close UNSETTLED/UNRESOLVED rows the one-shot 04:00 UTC D+1 settlement froze; decided grades are immutable; 972 passed locally (Py 3.11), 0 pyflakes in src/scripts. OWNER MUST PASTE TWO WORKFLOW GLOBS + ONE AGENTS.md WAIVER (exact lines in item 3) BEFORE TUE 2026-09-15 04:00 UTC DISPATCH for that run to commit the backlog recovery.**
...
3. **OWNER PASTE (the agent never edits workflow files; nothing under `.github/workflows/` was modified).** In `.github/workflows/forward_shadow.yml`:
   - **Line 54** — add these two alternatives inside the existing `find data/reports/shadow … \( … \)` group, immediately before `\) | xargs -r git add -f`:
     `-o -name 'settlement_supplement_*.json' -o -name 'settlement_supplement_*.sha256'`
   - **Line 55** — replace the whole line with:
   ...
   - **AGENTS.md** (Filesystem Separation, after the 2026-09-08 erratum waiver bullet) — append:
   ...
   - Until pasted, the pass still RUNS in the workflow and supplements survive in the 30-day run artifact, but nothing supplement-related is committed; oldest pending date (09-08) is last eligible at the 2026-09-22 04:00 UTC dispatch.
