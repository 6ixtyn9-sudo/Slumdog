...
2. **What changed (code + config + tests + docs; NO workflow file, NO frozen config, NO committed evidence).**
   - `src/slumdog/shadow_evaluator.py`: `TrackPolicy` / `track_policy()` (declaration-driven), `parse_kickoff_utc()` (both published shapes, UTC because captures pin `tz=0`; returns `None` rather than guessing), `_timing_classify_short_notice()` (per-event gate with five explicit rejection reasons), short-notice branch in `_emit_run()` / `evaluate_from_disk()` / `_blocked_run()`, `_validate_short_notice_timing()` in `load_shadow_declaration()`, and a symmetric guard so a standard declaration cannot write into the short-notice tree. Short-notice-only keys (`track`, `timing_contract`, `kickoff_utc`, `lead_minutes_at_decision`, `short_notice_timing_rejections`, `timing_track` in the input digest) follow the existing refresh-mode precedent: absent on standard runs, so those payloads/manifests/digests are byte-stable.
   ...
   - `config/shadow_evaluator_short_notice.json`: new declaration, `min_lead_minutes_before_kickoff` 120, all authorization gates still false, same `rule` / `anti_tuning` / `cohort_policy` blocks as the frozen declaration (asserted by test).
   ...
   - `scripts/forward_shadow_batch.py`: `run_short_notice_for_date` / `find_short_notice_run` / `summarise_short_notice_run`, `--skip-short-notice`, `config_rel` on `run_evaluator`, `shadow_subdir` threaded through the settlement finders/backlog, namespaced receipt keys (`short_notice`, `short_notice_settlement`, `short_notice_*` summary counters). Stage order: D+1 settlement → completion → delta settlement → refresh → **short-notice settlement + short-notice capture/evaluate** → forward pass. Fully isolated (never raises to the driver).
   ...
   - Tests: +92 (`tests/test_short_notice_track.py` 61, `tests/test_short_notice_batch.py` 31). `.gitignore` gained the short-notice tree with the same raw-body/archive exclusions. Docs: `docs/SHORT_NOTICE_TRACK.md` (new), `docs/STATE.md`, `docs/README.md`, `README.md`, this file.
3. **OWNER ACTION — workflow persist step (exact paste in `docs/SHORT_NOTICE_TRACK.md` §5).** Two gaps: (a) the pre-existing refresh-delta loss (`selections_delta_*`, `settlement_delta_*` never committed since 2026-09-22 — verified: zero delta files exist anywhere in the repo despite four daily refresh receipts), and (b) the new `data/reports/shadow_short_notice/` tree. `capture_short_notice_*.json` already matches the existing `capture_*.json` rule. Verify with `python scripts/check_workflow_evidence_globs.py` — exit 1 with 10 uncovered types today, exit 0 after the paste. The suite asserts the gap is a SUBSET of the documented list, so it is green before AND after; it fails only if a new uncovered artifact type appears.
4. **Honest status.** IMPLEMENTED AND TESTED LOCALLY. **No real short-notice run has been dispatched, and none has been settled** — no short-notice hit rate exists and none may be quoted. Bundling is not wired to the new tree; the completion pass still runs on the frozen tree only. With the current 04:00 UTC dispatch and a 120-minute lead, fixtures kicking off before ~06:00 UTC are refused by design; widening that is a dispatch-time decision the owner controls (no schedule was changed).
